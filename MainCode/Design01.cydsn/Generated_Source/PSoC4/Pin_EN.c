...
#include "Pin_EN.h"
    ...
    #define Pin_EN_SetP4PinDriveMode(shift, mode)  \
    do { \
        Pin_EN_PC =   (Pin_EN_PC & \
                                (uint32)(~(uint32)(Pin_EN_DRIVE_MODE_IND_MASK << \
                                (Pin_EN_DRIVE_MODE_BITS * (shift))))) | \
                                (uint32)((uint32)(mode) << \
                                (Pin_EN_DRIVE_MODE_BITS * (shift))); \
    } while (0)
#else
        ...
        #define Pin_EN_SetP4PinDriveMode(shift, mode)  \
        do { \
            Pin_EN_USBIO_CTRL_REG = (Pin_EN_USBIO_CTRL_REG & \
                                    (uint32)(~(uint32)(Pin_EN_DRIVE_MODE_IND_MASK << \
                                    (Pin_EN_DRIVE_MODE_BITS * (shift))))) | \
                                    (uint32)((uint32)(mode) << \
                                    (Pin_EN_DRIVE_MODE_BITS * (shift))); \
    ...
    *******************************************************************************/
    ...
    {
		Pin_EN_SetP4PinDriveMode(Pin_EN__0__SHIFT, mode);
    }
#endif
...
{
    uint8 drVal = (uint8)(Pin_EN_DR & (uint8)(~Pin_EN_MASK));
    drVal = (drVal | ((uint8)(value << Pin_EN_SHIFT) & Pin_EN_MASK));
    Pin_EN_DR = (uint32)drVal;
}
...
{
    return (uint8)((Pin_EN_PS & Pin_EN_MASK) >> Pin_EN_SHIFT);
}
...
{
    return (uint8)((Pin_EN_DR & Pin_EN_MASK) >> Pin_EN_SHIFT);
}
...
void Pin_EN_SetInterruptMode(uint16 position, uint16 mode)
{
    ...
    intrCfg =  Pin_EN_INTCFG & (uint32)(~(uint32)position);
    Pin_EN_INTCFG = intrCfg | ((uint32)position & (uint32)mode);
}
...
{
	uint8 maskedStatus = (uint8)(Pin_EN_INTSTAT & Pin_EN_MASK);
	Pin_EN_INTSTAT = maskedStatus;
    return maskedStatus >> Pin_EN_SHIFT;
}
